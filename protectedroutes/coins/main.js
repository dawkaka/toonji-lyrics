...
coinsRouter.post('/api/coins/buy-coins',validate,async (req,res) => {
 try{
   ...
   const user = await usersModel.findOne({userId},{userCoins:1})
   if(!user) return res.json({type:'ERROR',msg:'user not found'})
   let currCoins = (amount * 100) + user.userCoins
   await usersModel.updateOne({userId},{$set:{userCoins: currCoins}})
   await coinsPurchasesModel.updateOne({link},{$set:{completed: true,amount: amount, payerEmail: payerEmail}})
